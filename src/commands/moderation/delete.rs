...
pub async fn delete(ctx: Context<'_>, #[description = "# of Message to delete"] number: u64) -> Result<(), Error> {
    let num = number;
    let mut response = format!("The number you passed doesnt pass this comparison: 1 < {num} <= 100");
    if (1 < num) && (num <= 100) {
        let channel_id = ctx.channel_id();
        let messages = channel_id.messages(ctx.discord(), |retriever| retriever
            .before(ctx.id()).limit(num)).await.unwrap();
